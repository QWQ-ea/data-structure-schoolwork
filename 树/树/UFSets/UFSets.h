...
// 并查集的函数成员:
	...
	void HeightUnion(ElemType a,ElemType b); //根据高度大小合并a与b所在的等价类 
	int GetHeight(int p) const; //取以指定元素为根的树的高度 
};
...
void UFSets<ElemType>::HeightUnion(ElemType a, ElemType b)
{
	...
	if (r1 != r2 && r1 != -1) {
	int  temp = sets[r1].parent + sets[r2].parent;
	if(sets[r1].height>=sets[r2].height)
	{
		sets[r2].parent=r1;
		sets[r1].parent=temp;
		if(sets[r1].height==sets[r2].height) sets[r1].height+=1;
		}	
	else
	{
		 sets[r1].parent = r2;      
...
template <class ElemType>
int UFSets<ElemType>::GetHeight(int p) const
{
	if (p < 0 || p >= size)
		throw Error("元素不存在!");			// 抛出异常
	return sets[p].height;					// 返回高度	
}
#endif
