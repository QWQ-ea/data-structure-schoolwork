...
//  线索二叉树方法成员:
	...
	ThreadBinTreeNode<ElemType> *GetFront(ThreadBinTreeNode<ElemType> *p) const;
    ...
    void DeleteRightChild(ThreadBinTreeNode<ElemType> *p); 
	void InsertLeftChild(ThreadBinTreeNode<ElemType> *p,const ElemType &e);// 插入左孩子
	void DeleteLeftChild(ThreadBinTreeNode<ElemType> *p);   // 删除p的左子树 

	void InOrder(void (*Visit)(const ElemType &)) const;	// 二叉树的中序遍历	
	void FnOrder(void (*Visit)(const ElemType &)) const; 
...
template <class ElemType>
ThreadBinTreeNode<ElemType> *InThreadBinTree<ElemType>::GetFront(ThreadBinTreeNode<ElemType> *p) const
{
if(p->leftTag==1)
p=p->leftChild;
else
{
p=p->leftChild;
while(p->rightTag==0) p=p->rightChild;
}
return p;
}
...
template <class ElemType>
void InThreadBinTree<ElemType>::DeleteRightChild(ThreadBinTreeNode<ElemType> *p)
{
if(p==NULL||p->rightTag==1) return;
else
{
ThreadBinTreeNode<ElemType> *q;
q=p->rightChild;
while(q->rightTag==0) q=q->rightChild;
q=q->rightChild;
p->rightChild=q;
p->rightTag=1;
return;
...
template <class ElemType>
void InThreadBinTree<ElemType>::InsertLeftChild(ThreadBinTreeNode<ElemType> *p, const ElemType &e)
{	
if(p==NULL) return;
else
{
ThreadBinTreeNode<ElemType> *x=new ThreadBinTreeNode<ElemType>(e,p->leftChild,p,p->leftTag,1),*q;
if(p->leftTag==0)
{
q=p->leftChild;
while(q->rightTag==0) q=q->rightChild;
...
template <class ElemType>
void InThreadBinTree<ElemType>::FnOrder(void (*Visit)(const ElemType &)) const
{
ThreadBinTreeNode<ElemType> *p;
for(p=GetLast();p!=NULL;p=GetFront(p))
{
(*Visit)(p->data);
if (p->leftTag == 1)
	cout << "其左指针为线索指针，指向";
