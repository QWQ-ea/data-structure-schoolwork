...
// 求以r为根的二叉树的结点个数
	...
	void TransLevel(const BinTreeNode<ElemType> *r,int Level,void (*Visit)(const ElemType &)) const;
	void InorderwithStack(BinTreeNode<ElemType> *r,void (*Visit)(const ElemType &));
...
//  二叉树方法声明及重载编译系统默认方法声明:
	...
	int LeafCount() const;
	void TransLevel(int Level,void (*Visit)(const ElemType &));
	void InorderwithStack(void (*Visit)(const ElemType &));
};
...
leftleaf=LeafCount(r->leftChild);
rightleaf=LeafCount(r->rightChild);
...
template <class ElemType>
void BinaryTree<ElemType>::TransLevel(const BinTreeNode<ElemType> *r,int Level,void (*Visit)(const ElemType &)) const
{
if(r==NULL) return;
int level=1;
if(level<Level)
{
TransLevel(r->leftChild,Level-1,Visit);
TransLevel(r->rightChild,Level-1,Visit);
}
else (*Visit)(r->data);
}

template <class ElemType>
void BinaryTree<ElemType>::TransLevel(int Level,void (*Visit)(const ElemType &))
{
TransLevel(root,Level,Visit);
}

template <class ElemType>
void BinaryTree<ElemType>::InorderwithStack(BinTreeNode<ElemType> *r,void (*Visit)(const ElemType &))
{
LinkStack<BinTreeNode<ElemType> *> S;
BinTreeNode<ElemType> *currentadr=r;
if(currentadr==NULL) return;
start:
while(currentadr->leftChild!=NULL)
{
S.Push(currentadr);
currentadr=currentadr->leftChild;
}
goto L1;
L1:
(*Visit)(currentadr->data);
goto L2;
L2:
if(currentadr->rightChild!=NULL){currentadr=currentadr->rightChild;goto start;}
else if(S.IsEmpty()==false) {S.Pop(currentadr);goto L1;}
else goto L3;
L3:
return;
}
...
template <class ElemType>
void BinaryTree<ElemType>::InorderwithStack(void (*Visit)(const ElemType &))
{
InorderwithStack(root,Visit);
}
