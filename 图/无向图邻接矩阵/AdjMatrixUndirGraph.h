...
template <class ElemType>
void AdjMatrixUndirGraph<ElemType>::Getpath(ElemType &a,ElemType &b)
{
int x=GetOrder(a);
tag[x]=VISITED;
pathstack.Push(a);
...
template <class ElemType>
void AdjMatrixUndirGraph<ElemType>::findway(ElemType &a,ElemType &b)
{
 if(a==b)
 {
  cout<<"找到一条路径：";
 ...
 else
 {
	 ...
	 {
		 if(arcs[GetOrder(a)][i]==1&&tag[i]==UNVISITED)
		 {
			 tag[i]=VISITED;
