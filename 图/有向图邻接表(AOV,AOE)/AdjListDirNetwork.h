...
// 抽象数据类型方法声明及重载编译系统默认方法声明:
	...
	void statInDegree(int *InDegree)
	{
	for(int v=0;v<GetVexNum();v++) InDegree[v]=0;
	for(int v=0;v<GetVexNum();v++)
		for(int u=FirstAdjVex(v);u!=-1;u=NextAdjVex(v,u))
			InDegree[u]++;
	};
	Status TopSort()//AOV网络
	{
	int *InDegree=new int[GetVexNum()];
	int v,u,count=0,top=-1;
	ElemType e;
	statInDegree(InDegree);
	...
	delete []InDegree;
	if(count<GetVexNum()) return FAIL;
	else return SUCCESS;
	};
	Status CriticalPath()//AOE网络
	{
	int *InDegree=new int[GetVexNum()];	
	int *ve=new int[GetVexNum()];	
	int *vl=new int[GetVexNum()];	
	LinkQueue<int> q;
	LinkStack<int> s;
	int ee,el,u,v,count=0;
	ElemType e1,e2;
	for(v=0;v<GetVexNum();v++) ve[v]=0;
	statInDegree(InDegree);
	for(v=0;v<GetVexNum();v++)
	...
	for(v=FirstAdjVex(u);v!=-1;v=NextAdjVex(u,v))
	 {
		if(--InDegree[v]==0) q.EnQueue(v);
		if(ve[u]+GetWeight(u,v)>ve[v]) ve[v]=ve[u]+GetWeight(u,v);
	 }
	}
	delete []InDegree;
	if(count<GetVexNum())
	...
	s.Top(u);
	for(v=0;v<GetVexNum();v++) vl[v]=ve[u];
	while(!s.IsEmpty())
	{
	s.Pop(u);
	for(v=FirstAdjVex(u);v!=-1;v=NextAdjVex(u,v))
		if(vl[v]-GetWeight(u,v)<vl[u]) vl[u]=vl[v]-GetWeight(u,v);
		
	}
	for(u=0;u<GetVexNum();u++)
		for(v=FirstAdjVex(u);v!=-1;v=NextAdjVex(u,v))
	     {
		  ee=ve[u];el=vl[v]-GetWeight(u,v);
		  if(ee==el)
		  {
			GetElem(u,e1);
			GetElem(v,e2);
