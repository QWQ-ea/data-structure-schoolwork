...
// 邻接矩阵的数据成员:
	...
	LinkStack<ElemType> pathstack;          //路径stack
...
// 邻接矩阵类型的方法声明:
  ...
  void Getpath(ElemType &a,ElemType &b);
  void findway(ElemType &a,ElemType &b); //用于找出两点间的所有简单路径
  int Tag;
};
...
template <class ElemType, class WeightType>
void AdjMatrixUndirNetwork<ElemType,WeightType>::Getpath(ElemType &a,ElemType &b)
{
	int x=GetOrder(a);
	tag[x]=VISITED;
	pathstack.Push(a);
...
template <class ElemType, class WeightType>
void AdjMatrixUndirNetwork<ElemType,WeightType>::findway(ElemType &a,ElemType &b)
{
	if(a==b)
	{
		cout<<"找到一条路径：";
		pathstack.Traverse(Write<ElemType>);
		cout<<endl;
		ElemType e;
		pathstack.Pop(e);
		tag[GetOrder(e)]=UNVISITED;
		Tag=1;
	}
	else
	{
		...
		{
			if(arcs[GetOrder(a)][i]!=infinity&&arcs[GetOrder(a)][i]!=0&&tag[i]==UNVISITED)
			{
				tag[i]=VISITED;
				pathstack.Push(vertexes[i]);
				findway(vertexes[i],b);
			}
		}
		ElemType e;
		pathstack.Pop(e);
