...
// 操作结果：返回栈元素个数
{
	int count = 0;		// 计数器 
	Node<ElemType> *p; 
	for (p = top->next; p != NULL; p = p->next)		// 用p依次搜寻每个元素
		count++;		// 统计链栈中结点数
	return count;
}
...
{
	Node<ElemType> *p;		
	for (p = top->next; p != NULL; p = p->next)	// 用p依次搜寻当前栈的每个元素
...
//	将返回OVER_FLOW
{
	Node<ElemType> *p = new Node<ElemType>(e, top->next);
	if (p == NULL) 	// 系统内存耗尽
		return OVER_FLOW;
	else	{	// 操作成功
		top->next=p;
...
//	函数返回UNDER_FLOW
{
	if (IsEmpty())	// 栈空
		...
		Node<ElemType> *p = top->next;	// 保留原栈顶
		e = top->next->data;				// 用e返回栈顶元素
		top->next=p->next;			// 修改栈顶
		delete p;					// 删除原栈顶结点 
		return SUCCESS;
	}
}
...
{
	if (s.IsEmpty())	// s为空
		top =new Node<ElemType>;									// 构造一空栈
	else 	{	                                    // s非空,复制栈
		top = new Node<ElemType>;	// 生成当前栈项
