...
// 邻接矩阵类型的方法声明:
  ...
  void DFSpoquan();
  void DFSsearch(int x,int y,int gen);
};
...
template <class ElemType, class WeightType>
void AdjMatrixUndirNetwork<ElemType, WeightType>::DFSpoquan()
{
int v;
for(v=0;v<GetVexNum();v++) 
{
	for(int i=0;i<GetVexNum();i++) tag[i]=UNVISITED;
	DFSsearch(v,v,v);findx=0;findy=0;
}
Display();
}

template <class ElemType, class WeightType>
void AdjMatrixUndirNetwork<ElemType, WeightType>::DFSsearch(int x,int y,int gen)
{
tag[gen]=VISITED;
for(int i=0;i<GetVexNum();i++)
{
 if(arcs[x][i]!=0&&arcs[x][i]!=infinity&&i!=y&&i==gen)
 {
  if(arcs[x][i]>=arcs[findx][findy])
  {
	  findx=x;
	  findy=i;
  }	 
  cout<<"删除变为："<<findx<<" "<<findy<<endl;
  arcs[findx][findy]=infinity;
  arcs[findy][findx]=infinity;
  for(int j=0;j<GetVexNum();j++) tag[j]=UNVISITED;
  DFSsearch(gen,gen,gen);
  return;
 }
 if(arcs[x][i]!=0&&arcs[x][i]!=infinity&&i!=y&&tag[i]==UNVISITED)
 {
   if(arcs[x][i]>=arcs[findx][findy])
	 {
		 findx=x;
