...
// 顺序表的函数成员
	...
	void DelSameItem(const ElemType &e); //删除顺序表中与e相同的所有元素
	void Reverse(); //对顺序表原地倒置
	int BinSearch(int n,ElemType key);//二分查找迭代算法
	int BinSearch(int low,int high,ElemType key);//二分查找递归算法
	int FibSearch(int k,ElemType key);//斐波那切查找
	int ChaSearch(int n,ElemType key);//插值查找
};
...
template <class ElemType>
void SeqList<ElemType>::DelSameItem(const ElemType &e)
{
int i=0;
while(i<length)
{
...
template<class ElemType>
int SeqList<ElemType>::BinSearch(int low,int high,ElemType key)
{
int mid;
if(low>high) mid=-1;
else
 {
	mid=(low+high)/2;
	if(key<elems[mid]) mid=BinSearch(low,mid-1,key);
	else if(key>elems[mid]) mid=BinSearch(mid+1,high,key);
 }
return mid;
}

template<class ElemType>
int SeqList<ElemType>::BinSearch(int n,ElemType key)
{
int low=0,high=n-1;
int mid;
int m=0;
while(low<=high)
 {
	mid=(low+high)/2;
	m++;
	if(key==elems[mid]) {cout<<"查找成功次数为:"<<m<<endl;return mid;}
	else if(key<elems[mid]) high=mid-1;
	else low=mid+1;
 }
cout<<"查找失败次数为:"<<m<<endl;
return -1;	
...
template<class ElemType>
int SeqList<ElemType>::FibSearch(int k,ElemType key)
{
int n=Fib(k)-1;
int low=0,high=n-1;
int mid;
...
template<class ElemType>
int SeqList<ElemType>::ChaSearch(int n,ElemType key)
{
int low=0,high=n-1;
int mid;
int m=0;
while(low<high)
 {
	mid=low+1+(key-elems[low])*(high-low-1)/(elems[high]-elems[low]);
	m++;
	if(key==elems[mid]) {cout<<"查找成功次数为:"<<m<<endl;return mid;}
	else if(key<elems[mid]) high=mid-1;
	else low=mid+1;
 }
if(key==elems[low]) {cout<<"查找成功次数为:"<<m<<endl;return low;}
m++;
cout<<"查找失败次数为:"<<m<<endl;
return -1;
}
