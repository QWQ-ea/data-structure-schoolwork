...
//	辅助函数:
	int H(int row,int col) const;					// 散列函数
	int Collision(int row,int col, int i) const;	// 处理冲突的函数

public:
//  二叉树方法声明及重载编译系统默认方法声明:
  	ClosedHashTable(int size);			// 构造函数
  	~ClosedHashTable();								// 析造函数
  	void Traverse(void (*Visit)(const ElemType &)) const;	// 遍历散列表
	int Search(int row,int col,const KeyType &key) const ;	    // 查寻关键字为key的元素的下标 
	Status Insert(int row,int col,const ElemType &e);				// 插入元素e
	bool Delete(int row,int col,const KeyType &key);			// 删除关键字为key的元素
...
// 散列表类的实现部分
template <class ElemType, class KeyType>
int ClosedHashTable<ElemType, KeyType>::H(int row,int col) const
//操作结果: 返回散列地址
{
	return 2*(100*(row/10)+col/10);
}

template <class ElemType, class KeyType>
int ClosedHashTable<ElemType, KeyType>::Collision(int row,int col, int i) const
//操作结果: 返回第i次冲突的探查地址
{
	return (H(row,col) + i*i + maxSize) % maxSize;
}

template <class ElemType, class KeyType>
ClosedHashTable<ElemType, KeyType>::ClosedHashTable(int size)
...
template <class ElemType, class KeyType>
int ClosedHashTable<ElemType, KeyType>::Search(int row,int col,const KeyType &key) const
// 操作结果: 查寻关键字为key的元素的值,如果查找成功,返回元素在数组的下标,否则返回-1. 
{
	...
	while (c < maxSize &&	((tag[i] == USE && ht[i] != key) || tag[i] == DELETE))					
		...
		i = Collision(row,col, ++c);		// 求得下一个探查地址
...
template <class ElemType, class KeyType>
Status ClosedHashTable<ElemType, KeyType>::Insert(int row,int col,const ElemType &e)
...
template <class ElemType, class KeyType>
bool ClosedHashTable<ElemType, KeyType>::Delete(int row,int col,const KeyType &key)
// 操作结果: 删除关键字为key的数据元素,删除成功返回true,否则返回false
{
  int i = Search(row,col,key);
	if (i == -1)	                 // 查找失败,说明元素不存在。 
		return false; 
	else	{	                       
		tag[i] = DELETE;				     // 设置删除标记 
