...
//  二叉树方法声明及重载编译系统默认方法声明:
	...
	BinarySortTree(const ElemType &e);		// 构造以e为根的二叉树
	...
	BinarySortTree(const BinarySortTree<ElemType> &t);	// 复制构造函数
	BinarySortTree(BinTreeNode<ElemType> *r);			// 建立以r为根的二叉树
	BinarySortTree<ElemType> &operator=(const BinarySortTree<ElemType>& t); 	// 赋值运算符重载
...
// 操作结果：已知先序和中序序列构造二叉树
{
	...
	CreateBinarySortTree<ElemType>(r, pre, in, 0, n - 1, 0, n - 1);
		// 由先序和中序序列构造以r为根的二叉树 
	BinarySortTree<ElemType> *bt = new BinarySortTree<ElemType>(r);	// 生成二叉树
	return *bt;
}
...
{  ElemType e;
	if (r != NULL) 	{
		InOrder(r->leftChild);	// 首先遍历r的左子树
		q.Top(e);
		if(q.GetLength()==0||e<=r->data) q.Push(r->data);			// 再访问根结点r
		else return;
		InOrder(r->rightChild);	// 最后遍历r的右子树
	}
}

template<class ElemType>
bool BinarySortTree<ElemType>::BinarySorttreejudge(BinTreeNode<ElemType> *p)//判断以p为根节点的二叉树是否为二叉排序树
{
InOrder(p);
