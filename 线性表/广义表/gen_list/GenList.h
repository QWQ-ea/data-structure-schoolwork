...
// 抽象数据类型方法声明及重载编译系统默认方法声明:
	...
	Status Insert(int i,const ElemType &e);
	Status Insert(int i,GenList<ElemType> &subList);
	...
	Status transpose(GenListNode<ElemType> *hd);
	void show(char *s);
};
...
Status GenList<ElemType>::Insert(int i,const ElemType &e)
{
	...
	GenListNode<ElemType> *q = new GenListNode<ElemType>(ATOM,p);
	q->atom=e;
	pre->tLink=q;
	return SUCCESS;
   }
...
Status GenList<ElemType>::Insert(int i,GenList<ElemType> &subList)
{
	...
	else
	{
		...
		GenListNode<ElemType> *q = new GenListNode<ElemType>(LIST,p);
		q->hLink=subList.head;
		subList.head->ref++;
		pre->tLink=q;
		return SUCCESS;
	}
...
template<class ElemType>
Status GenList<ElemType>::transpose(GenListNode<ElemType> *hd)
{
if(GetLength()==0) return RANGE_ERROR;
else{
	GenListNode<ElemType> *pre=hd->tLink,*p=pre->tLink,*q=pre;
	pre->tLink=NULL;
	if(pre->tag==LIST) transpose(pre->hLink);
	while(p){
	q=pre;
    pre=p;
	p=pre->tLink;
...
template <class ElemType>
void GenList<ElemType>::show(char *s)
{
int i=0;
...
if(s[i]=='H') 
{ 
	if(p->tag==LIST) p=p->hLink;
	else if(p->tag==ATOM) break;
}
p=p->tLink;
i++;
}
...
if(s[i]=='T') cout<<"()"<<endl;
	...
	GenListNode<ElemType> *hd=new GenListNode<ElemType>(HEAD);
	hd->tLink=p;
	ShowHelp(hd);
	delete hd;
}
