...
// 稀疏矩阵三元组顺序表的函数成员： 
	...
	friend TriSparseMatrix<ElemType> operator+( TriSparseMatrix<ElemType> &a, TriSparseMatrix<ElemType> &b)
	{
		string s="行数或列数不相等";
		if(a.GetRows()!=b.GetRows()||a.GetCols()!=b.GetCols()) throw s;	
		TriSparseMatrix<ElemType> temp(a.rows,a.cols,a.maxSize);
		ElemType v,v1,v2;
		int i,j;
